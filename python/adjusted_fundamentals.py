...
# This file demonstrates requesting data for MSFT from our adjusted fundamentals API endpoint.
...
def load_data_to_csv():
  final_csv = '/tmp/new_constructs_adjusted_fundamentals_sample.csv'
  nc_endpoint = 'fundamentals/adjusted'
  ...
  print('Requesting datapoint list...')  
  datapoints = get_datapoints(headers)
  data = []
...
def get_datapoints(headers):
  datapoints = []
  
  url = '{}/{}'.format(API_BASE_URL, 'datapoint')
  response = requests.get(url=url, headers=headers)
  response_json = response.json()
  body = ast.literal_eval(response_json.get('body', {}))
  results = body.get('results')
  
  for datapoint in results.get('datapoints', {}):
    if 'Adjusted Fundamentals' in datapoint['endpoints']:
      datapoints.append(datapoint['name'])
